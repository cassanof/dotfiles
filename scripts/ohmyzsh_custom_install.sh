...
setup_zshrc() {
	...
	if [ -e "${HOME}/code/dotfiles" ]; then
		...
		echo "${BLUE}Dotfiles directory found! ${dotfilesdir}"
		...
		echo "${RED}Enter the directory of your DOTFILES repo clone${RESET}"
		read dotfilesdir

		case $dotfilesdir in
			"") echo "${BOLD}Givven no dotfiles directory!${RESET}" ;;
			*?*) echo "${BOLD}Changing dotfiles dir to: ${dotfilesdir}${RESET}" ;;
		esac
	fi
